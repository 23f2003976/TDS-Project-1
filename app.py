...
def get_db_connection():
    """Establishes a connection to the SQLite database with named column access."""
    try:
...
async def get_embedding(text, max_retries=3):
    ...
    while retries < max_retries:
        try:
            print(f"Getting embedding for text (length: {len(text)})")
            ...
            print("Sending request to embedding API")
...
async def find_similar_content(query_embedding, conn):
    try:
        print("Finding similar content in database")
        cursor = conn.cursor()
        results = []
        
        # Search discourse chunks
        print("Querying discourse chunks")
        cursor.execute("""
        ...
        print(f"Processing {len(discourse_chunks)} discourse chunks")
        processed_count = 0
        ...
        for chunk in discourse_chunks:
            try:
                ...
                if processed_count % 1000 == 0:
                    print(f"Processed {processed_count}/{len(discourse_chunks)} discourse chunks")
                    
            except Exception as e:
                logger.error(f"Error processing discourse chunk {chunk['id']}: {e}")
        
        # Search markdown chunks
        print("Querying markdown chunks")
        cursor.execute("""
        ...
        print(f"Processing {len(markdown_chunks)} markdown chunks")
        processed_count = 0
        ...
        for chunk in markdown_chunks:
            try:
                ...
                processed_count += 1
                if processed_count % 1000 == 0:
                    print(f"Processed {processed_count}/{len(markdown_chunks)} markdown chunks")
        ...
        print(f"Found {len(results)} relevant results above threshold")
        ...
        print(f"Returning {len(final_results[:MAX_RESULTS])} final results after grouping")
...
async def enrich_with_adjacent_chunks(conn, results):
    try:
        print(f"Enriching {len(results)} results with adjacent chunks")
        cursor = conn.cursor()
        enriched_results = []
...
async def process_multimodal_query(question, image_base64):
    ...
    try:
        print(f"Processing query: '{question[:50]}...', image provided: {image_base64 is not None}")
        if not image_base64:
            print("No image provided, processing as text-only query")
            return await get_embedding(question)
        
        print("Processing multimodal query with image")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, json=payload) as response:
                if response.status == 200:
                    ...
                    print(f"Received image description: '{image_description[:50]}...'")
        ...
        logger.error(traceback.format_exc())
        # Fall back to text-only query
        print("Falling back to text-only query due to exception")
        return await get_embedding(question)
...
def parse_llm_response(response):
    try:
        ...
        print(f"Parsed answer (length: {len(answer)}) and {len(links)} sources")
...
async def query_knowledge_base(request: Request):
    try:
        ...
        print(f"Received query request: question='{question[:50]}...', image_provided={image is not None}")
        ...
        try:
            # Process the query
            print("Processing query and generating embedding")
            ...
            print("Enriching results with adjacent chunks")
            ...
            if not result["links"]:
                print("No links extracted, creating from relevant results")
                links = []
                unique_urls = set()
